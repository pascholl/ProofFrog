...
| `get_divergence_point` | Find where two adjacent steps diverge in the canonicalization pipeline — primary tool for understanding why a hop fails |
...
3. For deeper diagnosis, use `get_divergence_point(proof_path, step_index)` to
   find which transformation causes the two games to diverge. This shows the
   game ASTs before and after the diverging transform.
4. Use `get_canonicalization_trace(proof_path, step_index)` to see which
   transforms fired at each iteration — helpful for understanding the
   simplification sequence.
5. Use `get_step_after_transform(proof_path, step_index, transform_name)` to
