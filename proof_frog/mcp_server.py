...
mcp: FastMCP = FastMCP(
    ...
    instructions=(
        ...
        "transforms fired, `get_divergence_point` to find where two adjacent steps "
        "diverge, and `get_step_after_transform` to inspect intermediate states."
    ),
)
...
@mcp.tool()  # type: ignore[misc, untyped-decorator]
def get_divergence_point(proof_path: str, step_index: int) -> Any:
    """Find where two adjacent game steps diverge in the canonicalization pipeline.

    Compares step_index and step_index+1 after each transformation to identify
    the point where their ASTs first diverge (or last match). This is the
    primary diagnostic tool for understanding WHY a hop fails.

    step_index is 0-based; the comparison is between step_index and step_index+1.

    Returns:
      success                  — bool
      equivalent               — whether the two games are actually equivalent
      current_canonical        — final canonical form of step_index (if not equivalent)
      next_canonical           — final canonical form of step_index+1 (if not equivalent)
      first_diverge_point      — {iteration, transform, current/next before/after}
      last_equal_point         — {iteration, transform} where they last matched
    """
    return _capture_divergence_point(
        _safe_resolve(proof_path), step_index, allowed_root=_directory
    )
