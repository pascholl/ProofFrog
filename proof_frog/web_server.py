...
def _capture_divergence_point(
    file_path: str, step_index: int, allowed_root: str | None = None
) -> dict[str, Any]:
    """Return divergence analysis between step_index and step_index+1."""
    buf = io.StringIO()
    try:
        with redirect_stdout(buf), redirect_stderr(buf):
            engine, proof_file = _setup_engine_for_proof(file_path, allowed_root)
        game1, err1 = _resolve_step_game(engine, proof_file, step_index)
        if game1 is None:
            return {"success": False, "error": err1}
        game2, err2 = _resolve_step_game(engine, proof_file, step_index + 1)
        if game2 is None:
            return {"success": False, "error": err2}
        suppress = io.StringIO()
        with redirect_stdout(suppress), redirect_stderr(suppress):
            result = engine.find_divergence(copy.deepcopy(game1), copy.deepcopy(game2))
        result["success"] = True
        return result
    except Exception as e:  # pylint: disable=broad-exception-caught
        return {
